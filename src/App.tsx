...
interface SelectedCellHint {
  r: number;
  c: number;
}

type TutorialTarget = 'none' | 'board' | 'input' | 'settings-button' | 'opponent' | 'directions' | 'category' | 'donate';
...
const useLowPerformanceMode = () => {
  ...
  useEffect(() => {
    ...
    const update = () => setIsLowPerformanceMode(mediaQuery.matches);
...
const AtmosphericBackground = ({ lowPerformanceMode }: { lowPerformanceMode: boolean }) => {
  if (lowPerformanceMode) {
    return (
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none bg-[#050505]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(239,68,68,0.12),transparent_35%),radial-gradient(circle_at_top_right,rgba(16,185,129,0.10),transparent_30%),radial-gradient(circle_at_bottom_left,rgba(37,99,235,0.10),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(245,158,11,0.10),transparent_28%)]" />
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none bg-[#050505]">
      ...
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none bg-[radial-gradient(circle,rgba(255,255,255,0.12)_0.5px,transparent_0.5px)] bg-[length:18px_18px]" />
    </div>
  );
};
...
export default function App() {
  ...
  const tutorialSeenKey = 'trips-traps-trull-guided-tour-seen';
  ...
  const [selectedCellHint, setSelectedCellHint] = useState<SelectedCellHint | null>(null);
  ...
  const donateButtonRef = useRef<HTMLAnchorElement>(null);
  ...
  useEffect(() => {
    const hasSeenTutorial = window.localStorage.getItem(tutorialSeenKey) === 'true';
  ...
  const estonianDirectionLabels = UI_STRINGS.est.directionAbbreviations;
  const lastMove = gameState.moves[gameState.moves.length - 1] ?? null;
  const directionLabelsByCell = useMemo(() => {
    const labels: Record<string, string> = {};

    Object.entries(directions as Record<string, { r: number; c: number }>).forEach(([key, value]) => {
      labels[`${value.r}-${value.c}`] = estonianDirectionLabels[key as keyof typeof estonianDirectionLabels] ?? '';
    });

    return labels;
  }, [directions, estonianDirectionLabels]);
  const oldestMovesByPlayer = useMemo(() => {
    const result: Partial<Record<Player, Move | null>> = {};

    (['X', 'O'] as const).forEach((player) => {
      const playerMoves = gameState.moves.filter((move) => move.player === player);
      result[player] = playerMoves.length === 3 ? playerMoves[0] : null;
    });

    return result;
  }, [gameState.moves]);
  const shouldDelayWinnerOverlay = gameState.winner === 'O' && gameState.opponent === 'bot';
  const winnerOverlayDelay = gameState.winner === 'Draw' ? 700 : shouldDelayWinnerOverlay ? 1500 : 900;
  const selectedHintDetails = useMemo(() => {
    if (!selectedCellHint) return null;

    const item = targetItems[selectedCellHint.r]?.[selectedCellHint.c];
    const direction = directionLabelsByCell[`${selectedCellHint.r}-${selectedCellHint.c}`];
    ...
    return {
      item,
      direction,
      exampleForward: `${item.eesti} ${direction}`,
      exampleReverse: `${direction} ${item.eesti}`
    };
  }, [directionLabelsByCell, selectedCellHint, targetItems]);
  const tutorialSteps = useMemo<TutorialStep[]>(() => {
    if (gameState.language === 'rus') {
      return [
        { id: 'welcome', target: 'none', title: 'Добро пожаловать', body: 'Это не обычные крестики-нолики. Чтобы сделать ход, нужно назвать предмет и место клетки по-эстонски.' },
        { id: 'board', target: 'board', title: 'Игровое поле', body: 'Сначала посмотри на сетку 3x3. У каждой клетки есть предмет и позиция. Нажатие по клетке поможет тебе начать ход.' },
        { id: 'input', target: 'input', title: 'Как сделать ход', body: 'После выбора клетки переходи к полю ввода и напиши по-эстонски предмет и позицию. Подсказки в самих клетках тоже всегда остаются на эстонском.' },
        { id: 'settings-button', target: 'settings-button', title: 'Все настройки здесь', body: 'Если хочешь сделать игру проще, сложнее или разнообразнее, открой настройки этой кнопкой.' },
        { id: 'opponent', target: 'opponent', title: 'С кем играть', body: 'Здесь можно выбрать: играть против бота или против другого человека на одном устройстве.' },
        { id: 'directions', target: 'directions', title: 'Разные системы направлений', body: 'Позиции клетки можно учить не только как верх-низ-центр, но и как компас, семья, эмоции, погода и другие наборы.' },
        { id: 'category', target: 'category', title: 'Меняй словарную тему', body: 'Здесь меняется набор слов: цвета, животные, фрукты, овощи и другие темы для тренировки словаря. Переводы смотри ниже в легенде под полем.' },
        { id: 'donate', target: 'donate', title: 'Поддержать проект', body: 'Если игра тебе помогает, внизу есть кнопка Buy me a coffee. Ей можно поддержать развитие проекта.' }
      ];
    }

    if (gameState.language === 'est') {
      return [
        { id: 'welcome', target: 'none', title: 'Tere tulemast', body: 'See ei ole tavaline trips-traps-trull. Käigu tegemiseks pead nimetama ruudu eseme ja asukoha eesti keeles.' },
        { id: 'board', target: 'board', title: 'Mängulaud', body: 'Alusta 3x3 mängulauast. Igal ruudul on oma ese ja asukoht. Ruudule vajutamine aitab sul käiku alustada.' },
        { id: 'input', target: 'input', title: 'Kuidas käiku teha', body: 'Pärast ruudu valimist mine sisestusvälja juurde ja kirjuta eseme nimi ning asukoht eesti keeles. Ka ruutude sees olevad vihjed jäävad alati eesti keelde.' },
        { id: 'settings-button', target: 'settings-button', title: 'Kõik seaded siin', body: 'Kui tahad mängu lihtsamaks, raskemaks või mitmekesisemaks teha, ava seaded selle nupuga.' },
        { id: 'opponent', target: 'opponent', title: 'Kellega mängida', body: 'Siin saad valida, kas mängid boti vastu või teise inimesega samas seadmes.' },
        { id: 'directions', target: 'directions', title: 'Erinevad suunakomplektid', body: 'Ruudu asukohti saad õppida mitte ainult üleval-all-keskel kujul, vaid ka kompassi, pere, emotsioonide, ilma ja teiste teemade kaudu.' },
        { id: 'category', target: 'category', title: 'Vaheta sõnavara teemat', body: 'Siin saad muuta sõnavara teemat: värvid, loomad, puuviljad, köögiviljad ja muud kategooriad. Tõlked leiad allpool legendist.' },
        { id: 'donate', target: 'donate', title: 'Toeta projekti', body: 'Kui see mäng sind aitab, siis allpool on nupp Buy me a coffee. Selle kaudu saad projekti toetada.' }
      ];
    }

    return [
      { id: 'welcome', target: 'none', title: 'Welcome', body: 'This is not regular tic-tac-toe. To make a move, you need to name the item and the square position in Estonian.' },
      { id: 'board', target: 'board', title: 'The Board', body: 'Start with the 3x3 board. Every square has an item and a position. Tapping a square helps you begin your move.' },
      { id: 'input', target: 'input', title: 'How To Move', body: 'After choosing a square, go to the input and type the item and the position in Estonian. The square labels also stay in Estonian on purpose.' },
      { id: 'settings-button', target: 'settings-button', title: 'All Settings Live Here', body: 'If you want to make the game easier, harder, or more varied, open settings with this button.' },
      { id: 'opponent', target: 'opponent', title: 'Choose Your Opponent', body: 'Here you can switch between playing against the bot or another human on the same device.' },
      { id: 'directions', target: 'directions', title: 'Different Direction Systems', body: 'You can learn positions not only as top-left and center, but also through compass, family, emotions, weather and more.' },
      { id: 'category', target: 'category', title: 'Change The Vocabulary Theme', body: 'This changes what words appear in the game: colors, animals, fruits, vegetables, and other themes. The translated help stays below in the legend.' },
      { id: 'donate', target: 'donate', title: 'Support The Project', body: 'If the game helps you, there is a Buy me a coffee button below. You can support the project there.' }
    ];
  }, [gameState.language]);
  const currentTutorialStep = tutorialSteps[tutorialStepIndex] ?? tutorialSteps[0];
  const tutorialActiveTarget = showTutorial ? currentTutorialStep.target : 'none';
  const startGuidedTour = useCallback(() => {
    setTutorialStepIndex(0);
    setShowTutorial(true);
  }, []);
  const closeGuidedTour = useCallback((markSeen = true) => {
    setShowTutorial(false);
    setTutorialStepIndex(0);
    setSettingsHighlight(null);
    setShowSettings(false);
    setTutorialTargetRect(null);
    if (markSeen) window.localStorage.setItem(tutorialSeenKey, 'true');
  }, [tutorialSeenKey]);
  ...
  const makeMove = (r: number, c: number, item: ItemInfo) => {
    ...
    setInputValue('');
    setError(null);
    setSelectedCellHint(null);
  };

  const handleCellHint = useCallback((r: number, c: number) => {
    if (gameState.board[r][c] || gameState.winner) return;

    setSelectedCellHint({ r, c });
    setError(null);
  ...
  useEffect(() => {
    ...
    setShowWinnerOverlay(false);
    const timer = window.setTimeout(() => setShowWinnerOverlay(true), winnerOverlayDelay);

    return () => window.clearTimeout(timer);
  }, [gameState.winner, winnerOverlayDelay]);

  useEffect(() => {
    if (!showTutorial) return;

    if (currentTutorialStep.target === 'opponent' || currentTutorialStep.target === 'directions' || currentTutorialStep.target === 'category') {
  ...
  const getTutorialTargetElement = useCallback(() => {
    switch (currentTutorialStep.target) {
      case 'board':
        return boardRef.current;
      case 'input':
        return inputSectionRef.current;
      case 'settings-button':
        return settingsButtonRef.current;
      case 'opponent':
        return opponentRef.current;
      case 'directions':
        return directionsRef.current;
      case 'category':
        return categoryRef.current;
      case 'donate':
        return donateButtonRef.current;
      default:
        return null;
    }
  ...
  useEffect(() => {
    ...
    if (currentTutorialStep.target === 'board' || currentTutorialStep.target === 'input' || currentTutorialStep.target === 'settings-button' || currentTutorialStep.target === 'donate') {
      const element = getTutorialTargetElement();
      if (!element) return;
  ...
  }, [currentTutorialStep.target, getTutorialTargetElement, showTutorial]);
  ...
  }, [getTutorialTargetElement, showTutorial]);
  ...
  const tutorialCardStyle = useMemo(() => {
    ...
    if (tutorialTargetRect.right + cardWidth + 24 < viewportWidth) {
      return {
        top: Math.max(20, Math.min(tutorialTargetRect.top - 16, viewportHeight - cardHeight - 20)),
        left: tutorialTargetRect.right + 18,
        width: cardWidth,
      } as React.CSSProperties;
    }

    if (tutorialTargetRect.left - cardWidth - 24 > 0) {
      return {
        top: Math.max(20, Math.min(tutorialTargetRect.top - 16, viewportHeight - cardHeight - 20)),
        left: tutorialTargetRect.left - cardWidth - 18,
        width: cardWidth,
      } as React.CSSProperties;
    }

    const centeredLeft = Math.max(16, Math.min(tutorialTargetRect.left + tutorialTargetRect.width / 2 - cardWidth / 2, viewportWidth - cardWidth - 16));
    const showAbove = tutorialTargetRect.bottom + cardHeight + 28 > viewportHeight && tutorialTargetRect.top > cardHeight + 40;
    const top = showAbove
      ? Math.max(16, tutorialTargetRect.top - cardHeight - 22)
      : Math.min(viewportHeight - cardHeight - 16, tutorialTargetRect.bottom + 24);

    return { top, left: centeredLeft, width: cardWidth } as React.CSSProperties;
  }, [currentTutorialStep.target, tutorialTargetRect]);
  ...
  }, [gameState.opponent, resetLocalGame, roomId]);

  const boardContent = useMemo(() => (
    <div className={`relative mb-8 p-4 bg-white/5 rounded-3xl border border-white/10 z-10 ${isLowPerformanceMode ? 'shadow-lg' : 'backdrop-blur-xl shadow-2xl'}`}>
        ...
        {gameState.winner && gameState.winningLine && (
          <svg className="absolute inset-0 w-full h-full z-30 pointer-events-none" viewBox="0 0 300 300">
            <motion.line
              ...
              style={isLowPerformanceMode ? undefined : { filter: `drop-shadow(0 0 10px ${gameState.winner === 'X' ? '#007AFF' : '#FF3B30'})` }}
            />
          </svg>
        )}
          ...
          row.map((cell, c) => (
            <motion.div
              ...
              animate={{
                ...
                boxShadow: cell
                  ? (isLowPerformanceMode ? `0 6px 18px ${cell.item.hex || '#007AFF'}33` : `0 15px 40px ${cell.item.hex || '#007AFF'}44`)
                  : 'inset 0 4px 20px rgba(0,0,0,0.6), inset 0 1px 1px rgba(255,255,255,0.1)',
            ...
            >
              {lastMove?.r === r && lastMove?.c === c && (
                <motion.div
                  ...
                  animate={{
                    opacity: showWinnerOverlay ? 0.2 : [0.35, 0.85, 0.35],
                    scale: showWinnerOverlay ? 1 : [0.96, 1.02, 0.96]
                  }}
                  transition={{ duration: shouldDelayWinnerOverlay ? 0.9 : 1.2, repeat: showWinnerOverlay ? 0 : Infinity, ease: "easeInOut" }}
                  className="absolute inset-0 border-4 border-white/50 z-20 rounded-2xl pointer-events-none shadow-[0_0_32px_rgba(255,255,255,0.45)]"
                />
              )}

              {showHints && (
                <span className={`absolute top-1 left-1 text-[0.5rem] md:text-[0.5625rem] font-bold uppercase font-mono z-10 leading-tight max-w-[80%] text-white px-1.5 py-0.5 bg-black/40 rounded-md border border-white/10 ${isLowPerformanceMode ? '' : 'backdrop-blur-sm'}`}>
                  {directionLabelsByCell[`${r}-${c}`] ?? ''}
                </span>
              )}
              ...
              <AnimatePresence mode="wait">
                {cell ? (
                  <motion.div
                    ...
                    exit={isLowPerformanceMode ? { scale: 0.92, opacity: 0, transition: { duration: 0.18 } } : { scale: 0, opacity: 0, filter: 'blur(10px)', transition: { duration: 0.3 } }}
                    transition={{
                      type: 'spring',
                      damping: isLowPerformanceMode ? 18 : 12,
                      stiffness: isLowPerformanceMode ? 260 : 200,
                      duration: isLowPerformanceMode ? 0.28 : 0.5
                    }}
                    className="flex flex-col items-center z-10"
                  >
                    <div className={`w-[66px] h-[66px] md:w-[98px] md:h-[98px] bg-white/90 rounded-full flex items-center justify-center border-2 border-white/20 ${isLowPerformanceMode ? 'shadow-lg' : 'backdrop-blur-sm shadow-[0_10px_20px_rgba(0,0,0,0.2)]'}`}>
                      {cell.player === 'X' ? (
                    ...
                    >
                      {showHints && (
                        <motion.div
                        className={`text-[0.625rem] font-bold uppercase font-mono opacity-100 text-center px-2 py-1 bg-black/60 rounded-md border border-white/10 text-white z-20 ${isLowPerformanceMode ? 'shadow-lg' : 'backdrop-blur-md shadow-xl'}`}
                        >
              ...
              {gameState.mode === '3-limit' && cell &&
               oldestMovesByPlayer[cell.player]?.r === r &&
               oldestMovesByPlayer[cell.player]?.c === c && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{
                    opacity: [0.1, 0.4, 0.1],
                    scale: isLowPerformanceMode ? 1 : [1, 1.05, 1],
                  }}
                  transition={{ duration: isLowPerformanceMode ? 2.2 : 1.5, repeat: Infinity, ease: "easeInOut" }}
              ...
              {!isLowPerformanceMode && cell && lastMove?.timestamp === cell.timestamp && (
                <motion.div
      ...
      <AnimatePresence>
        {gameState.winner && showWinnerOverlay && (
          <motion.div
            initial={{ opacity: 0, scale: 0.8 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.8 }}
            className={`absolute inset-0 bg-black/90 flex flex-col items-center justify-center z-50 p-8 text-center rounded-3xl border border-white/20 ${isLowPerformanceMode ? 'shadow-2xl' : 'backdrop-blur-xl shadow-[0_0_100px_rgba(0,122,255,0.3)]'}`}
          >
            <motion.div
            ...
            >
              {!isLowPerformanceMode && <div className="absolute inset-0 blur-2xl bg-[#FFCC00]/20 animate-pulse" />}
            ...
            {!isLowPerformanceMode && Array.from({ length: 12 }).map((_, i) => (
              <motion.div
                key={i}
                ...
                style={{ backgroundColor: ['#007AFF', '#FF3B30', '#FFCC00', '#34C759'][i % 4] }}
              />
            ))}
          </motion.div>
        )}
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white/20 relative overflow-hidden flex flex-col items-center p-4 md:p-8 transition-all duration-300">
      ...
      {gameState.category !== 'colors' && !isLowPerformanceMode && (
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
          ...
          <div className="flex gap-1 sm:gap-2 bg-[#1a1a1a] shadow-sm border border-white/10 rounded-full px-2 sm:px-4 py-1 sm:py-2">
            <Tooltip text={t.howToPlay}>
              <button onClick={startGuidedTour} className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center hover:bg-white/10 rounded-full transition-colors text-white/70 hover:text-white">
                <LucideInfo size={22} />
              </button>
            </Tooltip>
            ...
            <Tooltip text={t.settings}>
              <button 
                ...
                className={`w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center hover:bg-white/10 rounded-full transition-colors text-white/70 hover:text-white ${tutorialActiveTarget === 'settings-button' ? 'relative z-[72]' : ''}`}
              >
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className={`fixed inset-0 bg-black/60 z-50 flex items-end md:items-center justify-center p-0 md:p-4 ${isLowPerformanceMode ? '' : 'backdrop-blur-md'}`} 
          ...
          >
            <motion.div 
              ...
              className={`bg-[#1a1a1a] border-t md:border border-white/10 w-full max-w-md max-h-[90vh] overflow-hidden rounded-t-[2.5rem] md:rounded-[2.5rem] relative flex flex-col ${isLowPerformanceMode ? 'shadow-xl' : 'shadow-2xl'}`} 
            ...
            >
              ...
              <div className="overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <div className="flex items-center justify-between py-2">
                  ...
                  >
                    <motion.div 
                      ...
                      transition={isLowPerformanceMode ? { duration: 0.12 } : undefined}
                      className="absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm"
                    />
                  </button>
                </div>

                <div ref={opponentRef} className={tutorialActiveTarget === 'opponent' ? 'relative z-[72]' : undefined}>
                  <div className="flex items-center gap-2 mb-2">
                    <LucideUser size={14} className="text-[#007AFF]" />
                ...
                <div ref={directionsRef} className={`${tutorialActiveTarget === 'directions' ? 'relative z-[72]' : ''} transition-all duration-500 rounded-2xl ${settingsHighlight === 'directions' ? 'ring-2 ring-[#007AFF]/50 bg-[#007AFF]/10 p-3 -mx-3' : ''}`}>
                  <div className="flex items-center gap-2 mb-2">
                ...
                <div ref={categoryRef} className={`${tutorialActiveTarget === 'category' ? 'relative z-[72]' : ''} transition-all duration-500 rounded-2xl ${settingsHighlight === 'category' ? 'ring-2 ring-[#007AFF]/50 bg-[#007AFF]/10 p-3 -mx-3' : ''}`}>
                  <div className="flex items-center gap-2 mb-2">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/65">
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-[0.625rem] font-bold uppercase tracking-[0.25em] text-[#7CC3FF]">
                    {tutorialStepIndex + 1} / {tutorialSteps.length}
                  </p>
                  <h2 className="mt-2 text-2xl font-bold text-white">{currentTutorialStep.title}</h2>
                </div>
                <button onClick={() => closeGuidedTour(true)} className="rounded-full p-2 text-white/60 transition-colors hover:bg-white/5 hover:text-white">
                  <LucideX size={20} />
                </button>
              </div>

              <p className="mt-4 text-sm leading-relaxed text-white/75">{currentTutorialStep.body}</p>
              ...
              <div className="mt-4 flex items-center gap-2">
                {(['est', 'eng', 'rus'] as Language[]).map((lang) => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-2 text-xs font-bold uppercase transition-colors ${gameState.language === lang ? 'bg-[#007AFF] text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}
                  >
                    {lang}
                  </button>
                ))}
              ...
              {currentTutorialStep.id === 'donate' && (
                <p className="mt-3 text-sm text-[#FFD54A]">
                  {gameState.language === 'rus' ? 'Если обучение помогло, можно нажать Buy me a coffee.' :
                   gameState.language === 'est' ? 'Kui see õpetus aitas, võid vajutada Buy me a coffee.' :
                   'If this tour helped, you can tap Buy me a coffee.'}
                </p>
              )}

              <div className="mt-6 flex items-center justify-between gap-3">
                <button
                  onClick={() => closeGuidedTour(true)}
                  className="rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-white/70 transition-colors hover:bg-white/5 hover:text-white"
                >
                  {gameState.language === 'rus' ? 'Пропустить' : gameState.language === 'est' ? 'Jäta vahele' : 'Skip'}
                </button>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="rounded-xl border border-white/10 px-4 py-3 text-sm font-semibold text-white/70 transition-colors hover:bg-white/5 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    {gameState.language === 'rus' ? 'Назад' : gameState.language === 'est' ? 'Tagasi' : 'Back'}
                  </button>
                  <button
                    onClick={() => {
                    ...
                    className="rounded-xl bg-[#007AFF] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#0056B3]"
                  >
                    {tutorialStepIndex === tutorialSteps.length - 1
                      ? (gameState.language === 'rus' ? 'Завершить' : gameState.language === 'est' ? 'Lopeta' : 'Finish')
                      : (gameState.language === 'rus' ? 'Далее' : gameState.language === 'est' ? 'Edasi' : 'Next')}
                  </button>
                </div>
              </div>
            </motion.div>
      ...
      {/* Online Link */}
      {roomId && (
        <div className={`w-full max-w-2xl mb-8 p-4 bg-white/5 border border-white/10 rounded-2xl flex justify-between items-center relative z-10 ${isLowPerformanceMode ? 'shadow-lg' : 'backdrop-blur-md shadow-xl'}`}>
          <div className="flex flex-col">
      ...
      <div className="w-full max-w-2xl grid grid-cols-2 gap-4 mb-8 relative z-10">
        <div className={`p-6 rounded-2xl border transition-all duration-300 ${gameState.currentPlayer === 'X' ? `bg-white/10 border-[#007AFF] ${isLowPerformanceMode ? 'shadow-md scale-[1.02]' : 'shadow-lg scale-105'}` : 'bg-white/5 border-white/5 opacity-40'}`}>
        ...
        <div className={`p-6 rounded-2xl border transition-all duration-300 ${gameState.currentPlayer === 'O' ? `bg-white/10 border-[#FF3B30] ${isLowPerformanceMode ? 'shadow-md scale-[1.02]' : 'shadow-lg scale-105'}` : 'bg-white/5 border-white/5 opacity-40'}`}>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              {gameState.isBotThinking && (
                <motion.div 
                  ...
                  transition={{ duration: isLowPerformanceMode ? 1.4 : 1, repeat: Infinity }}
      ...
      {/* Board */}
      <div ref={boardRef} className={tutorialActiveTarget === 'board' ? 'relative z-[72]' : undefined}>
        {boardContent}
      </div>

      {/* Input Area */}
      <div ref={inputSectionRef} className={`w-full max-w-lg scroll-mt-24 ${tutorialActiveTarget === 'input' ? 'relative z-[72]' : ''}`}>
        {selectedHintDetails && !gameState.winner && (
          <div className="mb-4 rounded-2xl border border-[#007AFF]/30 bg-[#007AFF]/10 px-4 py-3 shadow-lg">
            <p className="text-sm font-semibold text-white">
              {gameState.language === 'rus' ? 'Чтобы занять эту клетку, напиши:' :
               gameState.language === 'est' ? 'Selle ruudu võtmiseks kirjuta:' :
               'To claim this square, type:'}
            </p>
            <p className="mt-2 text-base font-mono text-[#7CC3FF]">
              {selectedHintDetails.exampleForward}
            </p>
            <p className="mt-1 text-xs text-white/70">
              {gameState.language === 'rus' ? 'Подойдёт и обратный порядок:' :
               gameState.language === 'est' ? 'Sobib ka vastupidine järjekord:' :
               'Reverse order also works:'} <span className="font-mono">{selectedHintDetails.exampleReverse}</span>
            </p>
          </div>
        )}
        <form onSubmit={handleInput} className="relative group">
          <input
            ...
            placeholder={
              ...
              (selectedHintDetails?.exampleForward || t.inputPlaceholder)
            }
            className={`w-full bg-[#1a1a1a] border rounded-2xl py-5 pl-6 pr-40 text-base md:text-lg font-medium shadow-xl focus:outline-none focus:border-[#007AFF] focus:ring-2 focus:ring-blue-500/20 transition-all text-white ${selectedHintDetails ? 'border-[#007AFF]/40 placeholder:text-white/35' : 'border-white/10 placeholder:text-white/20'}`}
            autoFocus
          />
        ...
        {selectedHintDetails && !gameState.winner && (
          <div className="mt-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/75">
            <span className="font-semibold text-white">
              {gameState.language === 'rus' ? 'Выбрана клетка:' :
               gameState.language === 'est' ? 'Valitud ruut:' :
               'Selected square:'}
            </span>{' '}
            <span className="font-mono text-white/90">{selectedHintDetails.direction}</span>
          </div>
        )}
      ...
      <div className="mt-16 w-full max-w-4xl relative overflow-hidden rounded-3xl p-1 z-10">
        ...
        <div className={`relative bg-[#1a1a1a]/80 rounded-[22px] border border-white/10 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 ${isLowPerformanceMode ? 'shadow-xl' : 'backdrop-blur-xl shadow-2xl'}`}>
      ...
      <footer className="mt-12 w-full max-w-4xl border-t border-[#DADCE0] pt-8 pb-12 flex flex-col items-center gap-6">
        <div className="flex flex-wrap justify-center gap-4">
          <a 
            ...
            target="_blank" 
            rel="noopener noreferrer" 
            className={`donate-btn-bmc cookie-regular scale-75 md:scale-90 origin-center ${tutorialActiveTarget === 'donate' ? 'relative z-[72]' : ''}`}
          >
